        ...
        <div class="mt-2 bg-white dark:bg-gray-800 overflow-hidden shadow sm:rounded-lg py-4 px-4">
            <div class="flex mb-4">
                ...
                <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                    <input wire:model="search" type="text" class="shaddow appearance-none border rounded w-full py-2 px-3 text-blue-500 " aria-hidden="true" placeholder="Search">
                            ...
                            @endif
                            @if(session()->has('info'))
                                <div class="bg-green-500 border-2 border-green-600 rounded-b mb-2">
                                    <div>
                                        <h5 class="text-white font-bold ml-4">{{session('info')}}</h5>
                                    </div>
                                </div>
                            @endif
                            @if(session()->has('delete'))
                                <div class="bg-blue-500 border-2 border-red-600 rounded-b mb-2 ">
                                    <div>
                                        <h5 class="text-white font-bold ml-4">{{session('delete')}}</h5>
                                    </div>
                                </div>
                            @endif
            <table class="table-fixed w-full">
                ...
                <tbody class="text-center">
                    ...
                    <tr>
                        <td>{{$id}}</td>
                        <td> <img src="{{ url('storage/image/'.$product->image) }}" class="my-3 h-20 w-28">
                        <td>{{$product->name}}</td>
                        <td>{{$product->type}}</td>
                        <td>{{$product->qty}}</td>
